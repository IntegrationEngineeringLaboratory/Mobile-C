...
\noindent
(3) CMake: It can be obtained from http://www.cmake.org

\subsection{Setup of the\texttt{\_chrc} file}
Instructions on how to properly setup Ch for compilation can be found in Section 1.1 of the ChSDK guide located in 
CHHOME/docs/chsdk.pdf. CHHOME is the directory where Ch is installed.
\textbf{It is recommended to test the compiler on the examples provided in the ChSDK guide before trying to 
compile the Mobile-C library}.
All the compilers tested in Ch are already setup in the \texttt{\_chrc} file. To enable a specific one, uncomment the 
corresponding lines in the \texttt{\_chrc} file, as shown in the ChSDK guide.

Mobile-C uses CMake as a build system. To enable the use of CMake from Ch, add the following line to the \texttt{\_path} environment variable:

\begin{verbatim}
_path = stradd(_path , ``<CMAKEDIR>/bin;'');
\end{verbatim}

Where \texttt{<CMAKEDIR>} is the directory where CMake is installed. To test if the \texttt{\_path} variable has been properly updated, restart Ch and type \texttt{cmake} from a Ch command shell.
...
directory for {\bf 32-bit and 64-bit} Unix systems. Type them from within a Ch terminal. 
...
demos after compiling the Mobile-C library. The demos will
...
\item Ensure that your \texttt{\_chrc} file in your home directory is up to
 ...
 Additional information on how to setup Ch for compilation 
...
\item Unpack the Mobile-C source code. Ensure that you have write permissions
  ...
  If downloading Mobile-C from the git repository, this step can be skipped.
